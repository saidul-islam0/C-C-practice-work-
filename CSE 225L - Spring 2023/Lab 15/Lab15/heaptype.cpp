...
int GraphType<VertexType>::BreadthFirstSearch(VertexType startVertex, VertexType endVertex)
{
    ...
    distance[IndexIs(vertices, startVertex)] = 0;
        ...
        else
        {
            if (!IsMarked(vertex))
            {
                ...
                while (!vertexQ.IsEmpty())
                {
                    ...
                    int itemIndex = IndexIs(vertices, item);
                    int vertexIndex = IndexIs(vertices, vertex);
                    distance[itemIndex] = distance[vertexIndex] + 1;
                }
            }
        }
    ...
    return distance[IndexIs(vertices, endVertex)];
}
